...
int debug_mode; // is debug mode enabled?

int main(int argc, char* argv[]) {
  UNsdHelper nsdHelper;
  //nsdHelper.omain(argc, argv);  // This is the command line interface

  // That is the test part
  char * query;
  query = argv[argc-1];

  string ret = nsdHelper.find(query);
  printf("find(%s) return:'%s'\n", query, ret.c_str() );
/*
  //sleep(1);
  char qq[] = "trash._http._tcp.local";
  ret = nsdHelper.find( qq );
  printf("find(%s) return:'%s'\n", qq, ret.c_str() );
*/
  return 1;
}
...
int UNsdHelper::debug(const char* format, ...) {
  va_list args;
  int ret;
...
void UNsdHelper::usage(char* argv0) {
...
void UNsdHelper::fail(const char* msg) {
  perror(msg);
  exit(1);
}

void UNsdHelper::init_answer_list(FoundAnswerList* alist) {
  int i;
...
FoundAnswer* UNsdHelper::add_new_answer(FoundAnswerList* alist) {
  FoundAnswer* a;
  if(alist->length >= MAX_ANSWERS) {
    return NULL;
  }
  a = (FoundAnswer*) malloc(sizeof(FoundAnswer));
  if(!a) {
...
FoundAnswer* UNsdHelper::add_answer(FoundAnswerList* alist, FoundAnswer* a) {
  if(alist->length >= MAX_ANSWERS) {
    return NULL;
  }
...
void UNsdHelper::clear_answer_list(FoundAnswerList* alist) {
  int i;
  for(i=0; i < MAX_ANSWERS; i++) {
    if(alist->answers[i]) {
...
char* UNsdHelper::prepare_query_string(char* name) {
  int i;
  int count;
  int lastdot = 0;
  int len = strlen(name);
...
char* UNsdHelper::parse_hostname(char* hostname) {
  ...
  result = (char *) malloc(hostname_length);
  if(!result) {
    fail("failed to allocate memory for parsed hostname");
  }
...
mDNSMessage* UNsdHelper::mdns_make_message() {
  mDNSMessage* msg = (mDNSMessage*) malloc(sizeof(mDNSMessage));
  if(!msg) {
...
mDNSFlags* UNsdHelper::mdns_parse_header_flags(uint16_t data) {
  mDNSFlags* flags = (mDNSFlags*) malloc(sizeof(mDNSFlags));
  if(!flags) {
...
uint16_t UNsdHelper::mdns_pack_header_flags(mDNSFlags flags) {
  uint16_t packed = 0;
...
char* UNsdHelper::mdns_pack_question(mDNSQuestion* q, size_t* size) {
  char* packed;
  size_t name_length;
  uint16_t qtype;
  uint16_t qklass;
  ...
  *size = name_length + 2 + 2;
  ...
  // 1 char for terminating \0, 2 for qtype and 2 for qklass
  ...
  // The top bit of the qklass field is repurposed by mDNS
  ...
  qtype = htons(q->qtype);
  qklass = htons(q->qklass);

  memcpy(packed + name_length, &qtype, 2);
  memcpy(packed + name_length + 2, &qklass, 2);

  return packed;
}


void UNsdHelper::mdns_message_print(mDNSMessage* msg) {
...
int UNsdHelper::mdns_parse_question(char* message, char* data, int size) {
  mDNSQuestion q;
  char* cur;
  int parsed = 0;
  ...
  memcpy(&(q.qklass), cur, 2);
  q.qklass = ntohs(q.qklass);
  cur += 2;
  parsed += 2;
  if(parsed > size) {
    return 0;
...
int UNsdHelper::mdns_parse_rr_a(char* data, FoundAnswer* a) {
...
int UNsdHelper::mdns_parse_rr_ptr(char* message, char* data, FoundAnswer* a) {
  int parsed = 0;
...
int UNsdHelper::mdns_parse_rr_srv(char* message, char* data, FoundAnswer* a) {
  uint16_t priority;
  uint16_t weight;
  int parsed = 0;
...
uint16_t UNsdHelper::get_offset(char* data) {
  uint16_t offset;
...
char* UNsdHelper::parse_rr_name(char* message, char* name, int* parsed) {
  ...
  out = (char*) malloc(MAX_RR_NAME_SIZE);
  if(!out) {
...
void UNsdHelper::mdns_parse_rdata_type(char* message, mDNSResourceRecord* rr, FoundAnswer* answer) {

  switch(rr->type) {
  case DNS_RR_TYPE_A:
    mdns_parse_rr_a( (char*) rr->rdata, answer);
    break;
  case DNS_RR_TYPE_PTR:
    mdns_parse_rr_ptr(message, (char*) rr->rdata, answer);
    break;
  case DNS_RR_TYPE_SRV:
    mdns_parse_rr_srv(message, (char*) rr->rdata, answer);
    break;
...
void UNsdHelper::free_resource_record(mDNSResourceRecord* rr) {
  if(!rr) {
    return;
  }
  if(rr->name) {
...
int UNsdHelper::mdns_parse_rr(char* message, char* rrdata, int size, FoundAnswerList* alist, int is_answer) {
  ...
  // +10 because type, klass, ttl and rdata_lenth 
  ...
  memcpy(&(rr->klass), cur, 2);
  rr->klass = ntohs(rr->klass);
  cur += 2;
  parsed += 2;
...
int UNsdHelper::mdns_parse_message_net(char* data, int size, mDNSMessage* msg, FoundAnswerList* alist) {
...
mDNSMessage* UNsdHelper::mdns_build_query_message(char* query_str, uint16_t query_type) {
  ...
  question.qklass = 1; // klass for the internet (RFC 1035 section 3.2.4)
...
char* UNsdHelper::mdns_pack_message(mDNSMessage* msg, size_t* pack_length) {
  char* pack;
...
// returns -1 on error
int UNsdHelper::parse_received(char* data, int size, FoundAnswerList* alist) {
  int res;
  mDNSMessage msg;
...
// like -t 5
int UNsdHelper::get_arg(int argc, char* argv[], const char* key, char** val) {
  int i;
  for(i=1; i < argc; i++) {
...
int UNsdHelper::out_of_time(struct timeval* t) {
...
void UNsdHelper::print_answers(FoundAnswerList* alist) {
...
string UNsdHelper::getFirstAnswer(FoundAnswerList* alist) {
  ...
  for(i=0; i < alist->length; i++) {
    ...
    ret.append(inet_ntoa(answer->addr));
    ret.append(":");
    oss << answer->port;
    ret.append(oss.str());
    return ret;
...
int UNsdHelper::send_query(int sock, char* query_arg, uint16_t query_type) {
 ...
 */
int UNsdHelper::is_answer_complete(FoundAnswer* a) {
...
/*
  ...
  then send a query.
*/
void UNsdHelper::complete_answer(int sock, FoundAnswerList* alist, FoundAnswer* a) {
  FoundAnswer* b;  
  unsigned int i;
...
void UNsdHelper::complete_answers(int sock, char* query_arg, FoundAnswerList* alist) {
  unsigned int i;
  FoundAnswer* a;
...
void UNsdHelper::main_loop(int sock, char* query_arg, int min_answers, FoundAnswerList* alist, struct timeval* runtime) {
  struct sockaddr_in addr;
  socklen_t addrlen;
  int res;
  int resp;
  ...
  recvdata = (char*) malloc(DNS_BUFFER_SIZE);
  if(!recvdata) {
  ...
  while(1) {
    if((min_answers > 0) && (alist->completed_length >= (unsigned int) min_answers)) {
...
void UNsdHelper::make_srv_answer(int sock, char* query, FoundAnswerList* alist) {
...
int UNsdHelper::omain(int argc, char* argv[]) {
...
// Return service string like a: 'host:port' or ''
string UNsdHelper::find( char* query ) {
  ...
  make_srv_answer(sock, query, &alist);
  main_loop(sock, query, min_answers, &alist, &runtime);

  answer = getFirstAnswer(&alist);
  clear_answer_list(&alist);
